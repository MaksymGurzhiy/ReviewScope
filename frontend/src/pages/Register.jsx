...
import { ScanText, Mail, Lock, User, ArrowRight } from 'lucide-react'
import { useAuth } from '../lib/auth'

function Field({ id, label, icon, ...props }) {
  return (
    <div>
      <label htmlFor={id} className="ed-label">
        {label}
      </label>
      <div className="relative">
        ...
        <input id={id} className="ed-input pl-9" {...props} />
      </div>
    </div>
  )
}
      ...
      <aside className="hidden lg:flex flex-col justify-between p-12 border-r border-rule-200 bg-cream-50">
        <div>
          <p className="font-serif italic text-xl text-ink-900 leading-snug max-w-[28ch]">
            One upload — sentiment, aspects, keywords, themes.
          </p>
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-ink-500 mt-6">ReviewScope</p>
        </div>
        <p className="font-serif text-[15px] text-ink-600 italic">Structured like a briefing, not a spreadsheet.</p>
      </aside>
      ...
      <div className="grid place-items-center px-4 py-10">
        <div className="w-full max-w-[440px] animate-fade-in-up">
          ...
          >
            ...
            <h1 className="font-serif text-[32px] leading-tight text-ink-900 mt-2 mb-1">Create your account</h1>
            <p className="text-sm mb-6 text-ink-500">Start turning customer reviews into actionable insight.</p>
            ...
            <div className="mt-6 text-sm text-center text-ink-500">
              ...
              <Link to="/login" className="text-accent-600 hover:text-accent-700 underline underline-offset-2">
                Sign in
              </Link>
            </div>
          </div>
